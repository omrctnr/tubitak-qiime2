...
a<-subset_samples(ps_phylum, Condition =="Aseptic Loosening")
b<-subset_samples(ps_phylum, Condition =="Prosthetic Joint Infection")
...
mergedGP <- merge_samples(ps_filtered, "Condition")

# Use psmelt to obtain a long-format data.frame
phy <- mergedGP %>% tax_glom(taxrank = "Phylum")%>% 
  # transform_sample_counts(function(x)100* x / sum(x)) %>% 
  psmelt()
ocean <- subset(phy, Sample == "Aseptic Loosening")
ocean2 <- subset(phy, Sample == "Prosthetic Joint Infection")


phy

ps_phylum <- tax_glom(mergedGP, "Phylum")
